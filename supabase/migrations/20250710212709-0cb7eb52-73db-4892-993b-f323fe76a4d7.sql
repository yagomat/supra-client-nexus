-- Fase 1: Segurança Crítica - Mover cálculos de status para backend e rate limiting rigoroso (Corrigido)
...
BEGIN
  ...
  SELECT * INTO v_current_month_payment
  FROM public.pagamentos p
  WHERE p.cliente_id = p_cliente_id 
  ...
  IF v_current_month_payment.id IS NOT NULL AND v_cliente.status = 'ativo' THEN
    ...
    -- Se não encontrou, usar o pagamento do mês atual
    IF v_last_payment.id IS NULL THEN
      v_last_payment := v_current_month_payment;
    END IF;
    
    -- Calcular próxima data de vencimento
    IF v_last_payment.mes = 12 THEN
      v_next_due_date := make_date(v_last_payment.ano + 1, 1, LEAST(v_cliente.dia_vencimento, 31));
    ELSE
      v_next_due_date := make_date(v_last_payment.ano, v_last_payment.mes + 1, 
                                  LEAST(v_cliente.dia_vencimento, 
                                       EXTRACT(DAY FROM (make_date(v_last_payment.ano, v_last_payment.mes + 1, 1) + INTERVAL '1 month' - INTERVAL '1 day'))));
    END IF;
  ...
  ELSE
    ...
    IF v_days_diff > 0 THEN
      RETURN json_build_object(
        'type', 'overdue',
        'days', v_days_diff,
        'lastPaymentDate', make_date(v_last_payment.ano, v_last_payment.mes, 1),
        'nextDueDate', v_next_due_date
      );
    ELSIF v_days_diff = 0 THEN
      RETURN json_build_object(
...
-- 2. Função para calcular prioridade de ordenação

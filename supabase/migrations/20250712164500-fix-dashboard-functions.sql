...
-- Corrigir as funções do dashboard para usar check_rate_limit consolidada

-- Atualizar get_dashboard_critical_stats para usar a função consolidada
...
BEGIN
  -- Check rate limiting usando a função consolidada
  IF NOT public.check_rate_limit(user_id_param, 'dashboard_request', 50, 5) THEN
    RAISE EXCEPTION 'Rate limit exceeded for dashboard requests. Please wait a moment.';
  END IF;

  -- Log the request for audit usando a função consolidada
...
-- Atualizar get_dashboard_chart_data para usar a função consolidada também
...
BEGIN
  -- Check rate limiting usando a função consolidada (mais permissiva para gráficos)
  IF NOT public.check_rate_limit(user_id_param, 'dashboard_request', 100, 10) THEN
    RAISE EXCEPTION 'Rate limit exceeded for dashboard chart requests. Please wait a moment.';
  END IF;
